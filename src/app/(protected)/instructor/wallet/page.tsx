...
export default function InstructorWallet() {
  ...
  const getStatusBadge = (status: number) => {
    switch (status) {
      ...
      case 1:
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-green-600 bg-green-50 px-2 py-0.5 rounded-full border border-green-250">
            <CheckCircle2 className="h-3 w-3" /> Thành công
          </span>
        );
      case 2:
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-red-650 bg-red-50 px-2 py-0.5 rounded-full border border-red-100">
  ...
  if (loading) {
    return <div className="text-zinc-55/80 text-center py-10">Đang tải thông tin ví...</div>;
  }
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-2xl bg-zinc-900 p-6 text-white shadow-lg border border-zinc-805/85">
          ...
          <form onSubmit={handleWithdraw} className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-zinc-905/90 border-b border-zinc-150 pb-3">Yêu cầu rút tiền</h3>
            
            <div>
              <label className="block text-xs font-semibold text-zinc-650 mb-1">Chọn ngân hàng</label>
              <select
                value={bankName}
                onChange={(e) => setBankName(e.target.value)}
                className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-xs text-zinc-700 bg-white focus:outline-none focus:border-indigo-500"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-650 mb-1">Số tài khoản</label>
              <input
                type="text"
                required
                value={accountNumber}
                ...
                className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-xs text-zinc-950 focus:outline-none focus:border-indigo-500"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-zinc-655 mb-1">Tên chủ tài khoản</label>
              <input
                type="text"
                required
                value={accountName}
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-655 mb-1">Số tiền rút (đ)</label>
              <input
                type="number"
                required
                min={50000}
        ...
        <div className="lg:col-span-2 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm flex flex-col">
          <h3 className="text-sm font-bold text-zinc-950 border-b border-zinc-150 pb-3 mb-4">Lịch sử giao dịch</h3>
          
          {transactions.length === 0 ? (
            <div className="text-center py-20 text-zinc-400 text-xs">Chưa có lịch sử giao dịch nào phát sinh.</div>
          ) : (
            <div className="divide-y divide-zinc-150 overflow-y-auto max-h-[500px]">
              {transactions.map((tx) => (
                <div key={tx.walletTransactionId} className="py-4 flex items-start justify-between gap-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-450 mt-1">Giao dịch: {new Date(tx.createdAt).toLocaleString('vi-VN')}</p>
                    </div>
                  </div>
